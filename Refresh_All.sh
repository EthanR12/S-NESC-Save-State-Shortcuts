...
if echo $i | grep -q "^s"; then cat /tmp/saves.txt | grep "suspend\|cartridge" | grep -o "CLV-.-.\{5\}" | sort -u > \
/tmp/folder.txt; else cat /tmp/saves.txt | grep "png\|save.sram" | grep -o "CLV-.-.\{5\}" | sort -u > /tmp/folder.txt; fi
...
find /$media/hakchi/games/$i/*/$j -maxdepth 0 -type d 2>/dev/null | while read copy; do
	if grep -ql " ([0-9]*)" $copy/$j.desktop; then :; else if [ -d $final/$j ]; then :; else cp -r $copy $final; fi; fi
done

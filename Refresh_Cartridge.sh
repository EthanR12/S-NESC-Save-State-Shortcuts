...
if echo $i | grep -q "^s"; then cat /tmp/saves.txt | grep "cartridge" | grep -o "CLV-.-.\{5\}" | sort -u > \
/tmp/folder.txt; else cat /tmp/saves.txt | grep "save.sram" | grep -o "CLV-.-.\{5\}" | sort -u > /tmp/folder.txt; fi

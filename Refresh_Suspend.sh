...
if [ -d /media/hakchi ]; then
	...
	find /media/hakchi/saves -maxdepth 2 \( -name "*.png" -o -name "suspend*" -type d \) | while read file; do
	...
	find /var/lib/clover/profiles/0 -maxdepth 2 \( -name "*.png" -o -name "suspend*" -type d \) | while read file; do
		found="$(basename $(dirname $file))"
